...
<div id="wcmp-store-conatiner">
    <div class="wcmp-store-list-wrap">
        <?php
        if (isset($_GET['zone']) && isset($_GET['vendors'])) {
            $vendorIds = explode(',', $_GET['vendors']);
            $vendorIdsZ = get_vendor_ids_by_shipping_custom($_GET['zone']);
            $vendorIds = array_unique(array_merge($vendorIds, $vendorIdsZ));
